...
#' @param recursive Logical indicating whether to recursively copy directories. Default is \code{TRUE}.
...
#'
#' @export
mc_cp <- function(from, to, recursive = TRUE, flags="", verbose = FALSE) {
  if(recursive) {
    flags <- paste("-r", flags)
  }
  mc(paste("cp", flags, from, to), verbose = verbose)
}
