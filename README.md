...
[NCAR’s Stratus](https://ncar-hpc-docs.readthedocs.io/en/latest/) (based on
Western Digital S3), and [MinIO Servers](https://www.min.io/) (another open
...
Note that MinIO has moved the client to its closed-source AIStor product:
the official download host, `dl.min.io`, now returns "410 Gone" and the
[minio/mc](https://github.com/minio/mc) repository has been archived. To keep
existing workflows running, `install_mc()` installs the last release published
there (`RELEASE.2025-08-13T08-35-41Z`) from its GitHub release assets. That
client is frozen: it will receive no further upstream fixes. If you need a
different build, set `options(minioclient.version = )` to another release tag
or `options(minioclient.url = )` to the full URL of an `mc` binary.
