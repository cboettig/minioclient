...
[![CRAN status](https://www.r-pkg.org/badges/version/minioclient)](https://CRAN.R-project.org/package=minioclient)
...
At first use, all operations will attempt to install the client (after prompting) if not already installed.
Users can also install the minio client directly using `install_mc`.
...
Configuration of aliases is stored in a persistent configuration file, so aliases need be created only once on a given machine.
...
Providing helper methods allows tab-completion discovery of functions, R-based documentation, and improved handling of display behavior (e.g. `verbose=FALSE` by default on certain commands.)  See [official mc client docs](https://www.min.io//docs/minio/linux/reference/minio-mc.html?ref=docs-redirect) for details.
...
We can now use arbitrary `mc` commands (see [quickstart](https://www.min.io//docs/minio/linux/reference/minio-mc.html?ref=docs-redirect)).  For example, examine file information to confirm that eTags (md5sums here) match for these objects:
