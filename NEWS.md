...
* bug-fix: `install_mc()` could no longer download the client at all. MinIO
  moved 'mc' into its closed-source AIStor product, retired the community
  download host (every `https://dl.min.io/client/mc/release/` URL now returns
  "410 Gone"), and archived the [minio/mc](https://github.com/minio/mc)
  repository. `install_mc()` now installs the last release published there,
  `RELEASE.2025-08-13T08-35-41Z`, from its GitHub release assets, so existing
  workflows keep working. That client is frozen at that version and will
  receive no further upstream fixes. Set `options(minioclient.version=)` to
  install a different release tag, or `options(minioclient.url=)` to download
  from a mirror.
* `install_mc()` now downloads to a temporary file alongside the destination
  and only moves it into place on success, so a failed or truncated download
  can no longer leave a stub that later looks like an installed client.
  Failures raise an informative error, and a benign `download.file()` warning
  (some R versions warn about a length mismatch across the redirect to
  GitHub's CDN) no longer aborts an otherwise complete download.
* bug-fix: `mc()` and `mc_sql()` discarded the client's own error messages.
  `processx::run()` defaults to `error_on_status = TRUE` and threw before
  either function's `p$status != 0` check could run, so a failing `mc_*` call
  reported a generic "System command 'mc' failed" while the message `mc`
  wrote to stderr was lost. Both now run with `error_on_status = FALSE` -- in
  `mc()`, a caller passing that through `...` still wins -- and surface the
  underlying message. In `mc()` a timeout or interrupt, which now reaches the
  status check with `status = NA`, gets a descriptive message rather than an
  empty one.
